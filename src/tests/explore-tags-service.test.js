 ...
 */
 ...
 describe('user can create a tag in a tuit with REST API', () => {
     ...
     beforeAll(async () => {
         ...
         promises.push(deleteTuitByContent(mockedTuit.tuit));
         ...
         testTuit = await createTuit(author._id, mockedTuit);
         tid = testTuit._id;
     })
 ...
 describe("can retrieve all tags with REST API", () => { 
     ...
     beforeAll(async () => {
         ...
         */
         promises.push(deleteUsersByUsername(sonic.username));
         promises.push(await deleteTuitByContent(mockedTuit1.tuit));
         promises.push(await deleteTuitByContent(mockedTuit2.tuit));
         promises.push(await deleteTuitByContent(mockedTuit3.tuit));
         ...
         testTuit1 = await createTuit(uid, mockedTuit1);
         testTuit2 = await createTuit(uid, mockedTuit2);
         testTuit3 = await createTuit(uid, mockedTuit3);
         tid1 = testTuit1._id;
         tid2 = testTuit2._id;
         tid3 = testTuit3._id;
